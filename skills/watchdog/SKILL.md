...
allowed-tools: sandbox_setup sandbox_status run_cli run_code run_periodic skill_view skills_list
metadata:
  version: 1.4.0
  category: platform
...
per tick (no `while True` / long `time.sleep` inside the script). Obey host scope
rules when a project/session is bound.

## Setup
1. Write a tick script under `/workspace/cache/tick.py` with `run_code` (or `run_cli`).
