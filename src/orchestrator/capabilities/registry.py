...
class CapabilityRegistry:
    ...
    def all_tools(self) -> list[Any]:
        return [e.tool for e in self._by_name.values()]
...
# catalog_wrapper: sandbox status/setup + provision_cli (catalog → skill INSTALL → LLM).
# Exec still goes through run_skill_script, not free-form run_cli/run_code.
    ...
    for group in _GROUP_ORDER:
        for name in sorted(reg.names(groups=(group,), exclude_tags=_SKIP_AUTHORING_TAGS)):
            ...
            if (
                ...
                and entry.group == CapabilityGroup.SANDBOX
                and name not in _CATALOG_SANDBOX
            ):
                continue
            names.append(name)
    return " ".join(names)
