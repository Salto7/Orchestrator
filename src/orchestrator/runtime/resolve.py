...
# Last-resort LLM install recipe (see skills/tools-suggestor/references/PROMPT.md).
# Steps in ``install`` run in list order — put apt deps before git_clone / pip.
_PLANNER_INSTALL_SYSTEM = (
...
class InstallResolver(SharedService):
    ...
    def _parse_install_docs(self, text: str, *, binary: str) -> list[dict[str, Any]]:
        """Prefer explicit ``install:`` YAML fences; prose regexes are legacy fallback."""
        steps = install_steps_from_fences(text)
        if steps:
            return steps

        # Legacy: loose apt / pip / github mentions when no fence was present.
