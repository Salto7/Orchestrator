...
class LearnAuthoring(SharedService):
    ...
    def replan_tool(
        *,
        ...
        parts.append(
            ...
            + " Prefer custom/apt/github_release/git_clone in that order when "
            "appropriate.\n"
    ...
    def lint_skill(
        *, name: str, skill_md: str, files: dict[str, str] | None = None
        ...
        with tempfile.TemporaryDirectory(prefix=f"orchestrator-lint-{nm}-") as tmp:
    ...
    @staticmethod
    def lint_skill_dir(
        skill_ref: str, *, skills_root: Path | None = None
    ) -> dict[str, Any]:
        """Lint an on-disk skill directory under the skills root (or an explicit path)."""
        ref = (skill_ref or "").strip()
        if not ref:
            raise ValueError("skill name or path is required")
        root = Path(skills_root or get_config().skills_dir).resolve()
        path = Path(ref).expanduser()
        if not path.is_absolute():
            path = (
                Path(ref) if ("/" in ref or ref.startswith(".")) else root / ref
            ).resolve()
        if not path.is_dir():
            raise ValueError(f"skill directory not found: {path}")
        return SkillLinter.check(path)
